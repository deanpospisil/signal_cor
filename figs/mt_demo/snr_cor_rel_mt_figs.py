...
for ranking in [hi_r2_hi_snr, lo_r2_hi_snr]:
    for nm in ranking[:4]:
       ...
       if u1.mean('trial_tot').max()>u2.mean('trial_tot').max():
           colors=['orange', 'blue']
       else:
           colors=['blue', 'orange']
       ...
       plt.errorbar(x=u1.coords['dir'].values, 
                    y=u1.mean('trial_tot'),
                    yerr=u1.std('trial_tot')/(n**0.5), color=colors[0])
       
       plt.errorbar(x=u1.coords['dir'].values, 
                    y=u2.mean('trial_tot'),
                    yerr=u2.std('trial_tot')/(n**0.5), color=colors[1])
       #plt.ylim(0,110)
       ...
       plt.gca().text(-0.3, 1.15, string.ascii_uppercase[i-1], transform=plt.gca().transAxes,
                       size=20, weight='bold')
#       plt.legend(['unit 1', 'unit 2'])
plt.tight_layout()
plt.savefig('r2c_vs_r2_cithresh_examples_labs.pdf');
#%%
from scipy import  stats
...
plt.grid()
plt.savefig('r2c_vs_snr_cithresh.pdf');
...
for i, top_pref_theta in enumerate([ np.pi/4, np.pi*2, np.pi/4,]):
    ...
    if i ==1:
        plt.title('Pref. dir. $\in[0,360]$ (deg) ')

    elif i==0:
        plt.title('Pref. dir. $\in[0,45]$ (deg) ')
        plt.title('Tuning parameter \n sampling distribution\npref. dir.$\in[0,45]$')

        plt.xlabel('Pref. dir (deg)');
        plt.ylabel('Max-min\ntuning curve amp')
        plt.gca().text(x_lab, y_lab, 'A', transform=plt.gca().transAxes,
                       size=20, weight='bold')
    else:
    ...
    if tc[0,:,0,0].max()<tc[0,:,0,1].max():
        tc_colors = ['b', 'orange']
    else:
        tc_colors = ['orange', 'b']
    
    plt.plot(np.rad2deg(theta).squeeze(), tc[0,:,0,0], color=tc_colors[0])
    plt.plot(np.rad2deg(theta).squeeze(), tc[0,:,0,1].squeeze(), color=tc_colors[1])
    plt.ylim(0,200)
    if i==0:
       plt.ylabel('Expected spike count')
       plt.xlabel('Motion direction (deg)')
       plt.title('Example tuning curve pair')
       plt.gca().text(x_lab, y_lab, 'B', transform=plt.gca().transAxes,
                       size=20, weight='bold')
    else:
        plt.gca().set_xticklabels([])
        plt.gca().set_yticklabels([])
    ...
    if i==0:
       ...
       plt.gca().text(x_lab, y_lab, 'C', transform=plt.gca().transAxes,
                       size=20, weight='bold')
    ...
    if i==0:
       ...
       plt.gca().text(x_lab, y_lab, 'D', transform=plt.gca().transAxes,
                       size=20, weight='bold')
    else:
        plt.gca().set_xticklabels([])
        plt.gca().set_yticklabels([])
    ...
    if i==0:
       plt.title('Corr. of SNR & classic $r_s$ \n all experiments avg. $r=$'+ 
                 str(np.round(np.median(r_snr_rnaive), 2)))
       plt.xlabel('Spearman-r (SNR, $\hat{r^2}$)')
       plt.ylabel('# experiments (n='+str(n_sims) +')' )
       plt.gca().text(x_lab, y_lab, 'E', transform=plt.gca().transAxes,
                       size=20, weight='bold')
    else:
        plt.gca().set_xticklabels([])
        plt.title(' avg. $r=$'+ 
    ...
    if i==0:
       ...
       plt.annotate('n=' + str(n_neuron_pairs), (2e1, 1.2))
       plt.gca().text(x_lab, y_lab, 'F', transform=plt.gca().transAxes,
                       size=20, weight='bold')
    else:
        plt.gca().set_xticklabels([])
        plt.gca().set_yticklabels([])
    ...
    plt.xlim(-1, 1);
    if i==0:
       plt.title('Corr. of SNR & corrected $r_s$ \n all experiments avg. $r=$'+ 
                 str(np.round(np.median(r_snr_r_er), 2)))
       plt.xlabel('Spearman-r (SNR, $\hat{r}^2_{ER}$)')
       plt.ylabel('# experiments (n='+str(n_sims) +')' )
       plt.gca().text(x_lab, y_lab, 'G', transform=plt.gca().transAxes,
                       size=20, weight='bold')
    else:
        plt.gca().set_xticklabels([])
        plt.title('avg. $r=$' +  
...
plt.tight_layout()   
plt.savefig('snr_r2c_null_sim.pdf')
...
a = np.concatenate([np.cos(theta), np.sin(theta),np.ones((len(theta), 1))], -1)
...
pred_1 = np.dot(a, np.linalg.lstsq(a, 
                                   ...
                                   rcond=None)[0])
pred_null = np.dot(a, np.linalg.lstsq(a, resp.mean('trial').values.T, rcond=None)[0])
...
snr_null = (resp.mean('trial').var('stim')/
            resp.var('trial').mean('stim'))
ind = snr>1
fig = plt.figure(figsize=(4,3))
ax = plt.axes()
ax.plot(snr[:,0], pred_0_fit, mfc='none', marker='o', lw=0, color='orange');
ax.plot(snr[:,1], pred_1_fit,  mfc='none', marker='o', lw=0, color='red');
